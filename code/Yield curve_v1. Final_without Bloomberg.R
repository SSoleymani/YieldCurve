...
# Bloomberg_data = read_excel("C:/Users/soleysi/Desktop/Channel 1/Facebook_IBR_14 September 2018/US USD Composite Curves.xlsx", sheet = "10-26-2018")
...
output <- as.data.frame(matrix(data = NA, nrow= length(Term), ncol = 1 + length(OutputNames)))
...
output_midpoints = output %>%
  select(Term, ends_with(".Midpoint")) %>%
...
output_final = data.frame(Term = Term)
for (r in AllRating){
...
myoutput_midpoints = melt(data = output_midpoints_sorted, id = "Term")
